...
import { Helpers, Question, DropdownListModel, settings, QuestionDropdownModel } from "@bateman001/form-core";
...
export class SurveyQuestionDropdownBase<T extends Question> extends SurveyQuestionUncontrolledElement<T> {
  ...
  renderValueElement(dropdownListModel: DropdownListModel): React.JSX.Element | null {
    if (this.question.showInputFieldComponent) {
      return ReactElementFactory.Instance.createElement(this.question.inputFieldComponentName, {
        item: dropdownListModel.getSelectedAction(),
  ...
  protected renderInput(dropdownListModel: DropdownListModel): React.JSX.Element {
    ...
    const css = `${this.question.getControlClass()}`;
  ...
  createClearButton(): React.JSX.Element | null {
    ...
    return (
      <div className={this.question.cssClasses.cleanButton} style={style} onClick={this.clear} aria-hidden="true">
        <SvgIcon
