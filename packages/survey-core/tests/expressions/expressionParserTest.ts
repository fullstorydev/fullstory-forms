...
QUnit.test("Run dateDiff by years function", function (assert) {
  const runner = new ExpressionRunner(
    "dateDiff({birthday}, {currentDate}, 'years')"
  );
  const values = {
    birthday: new Date(1974, 10, 10),
    currentDate: new Date(2014, 11, 11),
  };
...
QUnit.test("Run dateDiff by months", function (assert) {
  const runner = new ExpressionRunner(
    "dateDiff({birthday}, {currentDate}, 'months')"
  );
  const values = {
...
QUnit.test("Run dateAdd() for months", function (assert) {
  ...
  assert.deepEqual(
    runner.run(values)?.toDateString(),
    new Date("2020-11-01").toDateString(),
    "November 1, 2020"
  );
  (<any>values).d1 = undefined;
  assert.equal(runner.run(values), null, "a value is undefined");
...
QUnit.test(
  "Compare date with today, settings.useLocalTimeZone = false",
  function (assert) {
...
QUnit.test(
  "ExpressionRunner: sumInArray with conditional logic",
  function (assert) {
...
QUnit.test(
  "ExpressionRunner: sumInArray with conditional logic as operand",
  function (assert) {
    var runner = new ExpressionRunner("sumInArray({a}, 'val1', {val2} > 3)");
    var values = {
      a: [
...
QUnit.test(
  "ExpressionRunner: countInArray with conditional logic & strings",
  function (assert) {
    var runner = new ExpressionRunner(
      "sumInArray({a}, 'val1', {val2} <> 'item1')"
    );
    var values = {
      a: [
        { val1: 1, val2: "item2" },
...
QUnit.test(
  "ExpressionRunner: countInArray, value as string",
  function (assert) {
    var runner = new ExpressionRunner("countInArray({a}, 'val1')");
...
QUnit.test(
  "ConditionRunner, iif with empty parameter, Bug#2732",
  function (assert) {
...
QUnit.test(
  "Async function inside the sync function, Bug#8732",
  function (assert) {
    function asyncFunc(params: any): any {
      this.returnResult(params[0] * 3);
      return false;
...
QUnit.test(
  "Async function inside the async function, Bug#8742",
  function (assert) {
...
QUnit.test("Async & sync nested functions, Bug #8732 #8742", function (assert) {
  ...
  const expression =
    "asyncFunc2({e}, asyncFunc1({a}, asyncFunc1({b}, syncFunc2({c}, {d}))))" +
...
QUnit.test("Several async functions in expression", function (assert) {
  ...
  runner.onBeforeAsyncRun = (id: number) => {
    idBefore = id;
  };
  runner.onAfterAsyncRun = (id: number) => {
    idAfter = id;
  };
  var runnerResult = null;
  runner.onRunComplete = function (result: any) {
...
QUnit.test(
  "ExpressionRunner: fix incorrect JavaScript multiplication",
  function (assert) {
...
QUnit.test(
  "ExpressionRunner: fix incorrect JavaScript summary",
  function (assert) {
...
QUnit.test(
  "ExpressionRunner: fix incorrect JavaScript summary in sum function",
  function (assert) {
...
QUnit.test(
  "ExpressionRunner: fix incorrect JavaScript summary in avg function",
  function (assert) {
...
QUnit.test(
  "ExpressionRunner: fix incorrect JavaScript summary in sumInArray function",
  function (assert) {
    var runner = new ExpressionRunner("sumInArray({a}, 'val1')");
    ...
    assert.equal(
      runner.run(values),
      0.9,
      "It works correctly, sumInArray func"
    );
  }
);
QUnit.test(
  "ExpressionRunner: fix incorrect JavaScript summary in avgInArray function",
  function (assert) {
    ...
    assert.equal(
      runner.run(values),
      0.45,
      "It works correctly, avgInArray func"
    );
  }
);
QUnit.test(
  "ExpressionRunner: sumInArray function, float and string",
  function (assert) {
    var runner = new ExpressionRunner("sumInArray({a}, 'val1')");
    var values = {
      a: [{ val1: 0.3 }, { val1: "10" }, { val1: "0.6" }, { val1: "abc" }],
    };
    assert.equal(
      runner.run(values),
      10.9,
...
QUnit.test(
  "ExpressionRunner: minInArray function, float and string",
  function (assert) {
...
QUnit.test(
  "ExpressionRunner: maxInArray function, float and string",
  function (assert) {
...
QUnit.test("Operand.isEqual()", function (assert) {
  ...
  assert.equal(
    compareOperands("func({a}) < 3", "func({b}) < 3"),
    false,
    "#4.1"
  );
  assert.equal(
    compareOperands("func({a, b, 5}) < 3", "func({a, b, 5}) < 3"),
    true,
    "#4.2"
  );
...
QUnit.test(
  "ExpressionRunner: apply custom converter, #8634",
  function (assert) {
...
QUnit.test(
  "ExpressionRunner: do not convert to number extreme large strings",
  function (assert) {
