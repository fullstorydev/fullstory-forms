...
import { QuestionMatrixDynamicModel } from "@bateman001/fullstory-form-core";
...
export class SurveyQuestionMatrixDynamic extends SurveyQuestionMatrixDropdownBase {
  ...
  protected renderNoRowsContent(cssClasses: any): React.JSX.Element {
    ...
    const btn: React.JSX.Element | undefined = this.matrix.renderedTable.showAddRow
  ...
  protected renderAddRowButton(cssClasses: any, isEmptySection: boolean = false): React.JSX.Element {
  ...
  protected renderElement(): React.JSX.Element {
    ...
    const addButton = (
      <button
        ...
        onClick={this.matrix.isDesignMode ? undefined : this.handleOnRowAddClick}
      >
        {addRowText}
        <span className={this.props.cssClasses.iconAdd} />
      </button>
    );
    return this.props.isEmptySection ? addButton : <div className={this.props.cssClasses.footer}>{addButton}</div>;
  }
}
