...
export class SurveyFilePreview extends SurveyElementBase<{ question: QuestionFileModel }, {}> {
  ...
  protected renderElement(): React.JSX.Element | null {
    const content = this.question.supportFileNavigator
      ? this.question.renderedPages.map((page: any, index: number) => {
          return <SurveyFilePage page={page} question={this.question} key={page.id}></SurveyFilePage>;
        })
      : this.question.previewValue.map((item: any, index: number) => {
          return <SurveyFileItem item={item} question={this.question} key={index}></SurveyFileItem>;
        });
