...
import { QuestionRadiogroupModel, ItemValue, Base, SurveyModel } from "@bateman001/fullstory-form-core";
...
export class SurveyQuestionRadiogroup extends SurveyQuestionElementBase {
  ...
  protected getColumnedBody(cssClasses: any) {
    return <div className={cssClasses.rootMultiColumn}>{this.getColumns(cssClasses)}</div>;
  }
  protected getColumns(cssClasses: any) {
    var value = this.getStateValue();
    return this.question.columns.map((column: Array<ItemValue>, ci: number) => {
      var items = column.map((item: any, ii: number) => this.renderItem(item, value, cssClasses, "" + ci + ii));
      return (
        <div
  ...
  private renderItem(item: ItemValue, value: any, cssClasses: any, index?: string): React.JSX.Element {
