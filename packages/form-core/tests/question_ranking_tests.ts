...
QUnit.test(
  "Ranking: check removing other from visibleChoices",
  function (assert) {
    var survey = new SurveyModel({
      elements: [
        {
...
QUnit.test(
  "Ranking: Carry Forward and unrankIfChoicesChanged",
  function (assert) {
    var survey = new SurveyModel({
      elements: [
        {
    ...
    var checkboxQuestion = <QuestionCheckboxModel>(
    ...
    assert.equal(
      rankingQuestion.value.length,
      0,
      "unrank items after choices changed"
    );
    ...
    assert.equal(
      rankingQuestion.value.length,
      2,
      "after user's arrengement unrank should stop working"
    );
  }
);
...
QUnit.test("Ranking: design mode", function (assert) {
  ...
  q.handleKeydown(
    <any>{ key: "ArrowUp", preventDefault: preventDefault },
    q.choices[1]
  );
  ...
  q.handleKeydown(
    <any>{ key: "ArrowDown", preventDefault: preventDefault },
    q.choices[1]
  );
  assert.equal(upCalled, 1);
...
QUnit.test(
  "Ranking: selectToRank key navigation with animation",
  function (assert) {
    var survey = new SurveyModel({
      elements: [
        {
    ...
    assert.deepEqual(
      q.unRankingChoices.map((item) => item.value),
      ["a", "c"]
    );
    assert.deepEqual(
      q.rankingChoices.map((item) => item.value),
      ["b"]
    );
...
QUnit.test(
  "Ranking: check renderedRankingChoices are updated when rankingChoices changing",
  (assert) => {
    ...
    assert.deepEqual(
      question.renderedRankingChoices.map((item) => item.value),
      ["a", "b", "c"]
    );
...
QUnit.test(
  "Ranking: check rendered rankingChoices and unRankingChoices are updated when rankingChoices and unRankingChoices changing",
  (assert) => {
    const survey = new SurveyModel({
      elements: [
        {
    ...
    const question = <QuestionRankingModel>survey.getAllQuestions()[0];
    assert.deepEqual(
      question.renderedUnRankingChoices.map((item) => item.value),
      ["a", "b", "c"]
    );
    assert.deepEqual(
      question.renderedRankingChoices.map((item) => item.value),
...
QUnit.test("Ranking: test animation options", (assert) => {
  ...
  //test enterOptions
  assert.equal(
    rcAnimationOptions.getEnterOptions(question.unRankingChoices[0]).cssClass,
    "sv-ranking-item--animate-item-adding"
  );
  assert.equal(
    ucAnimationOptions.getEnterOptions(question.rankingChoices[0]).cssClass,
    "sv-ranking-item--animate-item-adding"
  );

  assert.equal(
    rcAnimationOptions.getEnterOptions(question.rankingChoices[0]).cssClass,
    "sv-ranking-item--animate-item-adding"
  );
  assert.equal(
    ucAnimationOptions.getEnterOptions(question.unRankingChoices[0]).cssClass,
    "sv-ranking-item--animate-item-adding"
  );
  ...
  assert.equal(
    rcAnimationOptions.getLeaveOptions(question.unRankingChoices[0]).cssClass,
    "sv-ranking-item--animate-item-removing"
  );
  assert.equal(
    ucAnimationOptions.getLeaveOptions(question.rankingChoices[0]).cssClass,
    "sv-ranking-item--animate-item-removing"
  );

  assert.equal(
    rcAnimationOptions.getLeaveOptions(question.rankingChoices[0]).cssClass,
    "sv-ranking-item--animate-item-removing"
  );
  assert.equal(
    ucAnimationOptions.getLeaveOptions(question.unRankingChoices[0]).cssClass,
    "sv-ranking-item--animate-item-removing"
  );
  ...
  assert.equal(
    rcAnimationOptions.getReorderOptions(question.rankingChoices[0], false)
      .cssClass,
    ""
  );
  assert.equal(
    rcAnimationOptions.getReorderOptions(question.rankingChoices[0], true)
      .cssClass,
    ""
  );

  assert.equal(
    rcAnimationOptions.getReorderOptions(question.rankingChoices[1], false)
      .cssClass,
    "sv-dragdrop-moveup"
  );
  assert.equal(
    rcAnimationOptions.getReorderOptions(question.rankingChoices[1], true)
      .cssClass,
...
QUnit.test(
  "Ranking: isItemSelected() returns always true for optimization",
  function (assert) {
    let result;
    let dragStartTargetNode;
    ...
    const rankingQuestion = <QuestionRankingModel>(
      survey.getQuestionByName("q1")
    );
    assert.equal(
      rankingQuestion.isItemSelected(rankingQuestion.choices[0]),
      true,
      "#1"
    );
...
QUnit.test("Ranking: separateSpecialChoices ", function (assert) {
  ...
  assert.ok(
    Serializer.findProperty("checkbox", prop).visible,
    "checkbox separateSpecialChoices is visible"
  );
  assert.notOk(
    Serializer.findProperty("ranking", prop).visible,
    "ranking separateSpecialChoices is invisible"
  );
});
...
QUnit.test(
  "Ranking: strict compare with not equal, Bug#7298",
  function (assert) {
    var survey = new SurveyModel({
      elements: [
        { type: "ranking", name: "q1", choices: ["a", "b", "c"] },
...
QUnit.test("Ranking: disabledItem", function (assert) {
  ...
  assert.equal(
    rankingQuestion.canStartDragDueItemEnabled(disabledItem),
    false,
    "can't start drag disabled item"
  );
  assert.equal(
    rankingQuestion.getItemTabIndex(disabledItem),
    undefined,
    "can't move disabled item via keyboard"
  );
});

QUnit.test(
  "Ranking: disabledItem with selectToRank and maxSelectedChoices",
  function (assert) {
    var survey = new SurveyModel({
      elements: [
        {
...
// selectToRankEnabled
function createRankingQuestionModel(
  ...
  // Create a survey with the ranking question
  const survey = new SurveyModel({
    elements: [json],
  });
  survey.updateButtonValuesCallBack = () => {};

  return survey.getQuestionByName("qr1") as QuestionRankingModel;
}
...
QUnit.test("selectToRankEnabled : initial", function (assert) {
  ...
  assert.equal(
    questionModel.unRankingChoices.length,
    3,
    "unRankingChoices count"
  );
...
QUnit.test("selectToRankEnabled : defaultValue", function (assert) {
  ...
  const questionWithDefaultValueModel = createRankingQuestionModel(
    selectToRankEnabled,
    withDefaultValue
  );
  assert.equal(
    questionWithDefaultValueModel.unRankingChoices.length,
    1,
    "unRankingChoices count"
  );
  assert.equal(
    questionWithDefaultValueModel.rankingChoices.length,
    2,
    "rankingChoices count"
  );
});

QUnit.test(
  "selectToRankEnabled : checkMaxSelectedChoicesUnreached",
  function (assert) {
    const selectToRankEnabled = true;
    ...
    assert.equal(
      questionModel.checkMaxSelectedChoicesUnreached(),
      true,
      "without MaxSelectedChoices"
    );
    ...
    assert.equal(
      questionModel.checkMaxSelectedChoicesUnreached(),
      false,
      "MaxSelectedChoices limit reached"
    );
  }
);

QUnit.test(
  "selectToRankEnabled : checkMaxSelectedChoices and handleKeydownSelectToRank",
  function (assert) {
    const selectToRankEnabled = true;
    const withDefaultValue = true;
    const questionModel = createRankingQuestionModel(
    ...
    questionModel.handleKeydownSelectToRank(
      fakeEvent,
      questionModel.unRankingChoices[0],
      " ",
      false
    );

    assert.equal(
      questionModel.value.length,
      2,
      "can't add due to MaxSelectedChoices"
    );

    questionModel.handleKeydownSelectToRank(
      fakeEvent,
      questionModel.rankingChoices[0],
      " ",
      false
    );
    ...
    assert.equal(
      questionModel.value.length,
      1,
      "unrank with MaxSelectedChoices"
    );
  }
);
...
QUnit.test("Ranking: renderedSelectToRankAreasLayout", function (assert) {
  ...
  assert.equal(
    questionModel.renderedSelectToRankAreasLayout,
    "horizontal",
    "default"
  );
  ...
  assert.equal(
    questionModel.selectToRankAreasLayout,
    "vertical",
    "set vertical"
  );
  assert.equal(
  ...
  assert.equal(
    questionModel.renderedSelectToRankAreasLayout,
    "vertical",
    "'vertical' always on mobile"
  );
});

QUnit.test(
  "Ranking: check SelectToRankEmptyRankedAreaText & SelectToRankEmptyUnrankedAreaText properties",
  (assert) => {
    let json: any = {
      questions: [
        {
    ...
    const rankedProp = Serializer.getProperty(
      "ranking",
      "selectToRankEmptyRankedAreaText"
    );
    const unrankedProp = Serializer.getProperty(
      "ranking",
