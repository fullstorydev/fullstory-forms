...
QUnit.test(
  "Serializer.getObjPropertyValue/setObjPropertyValue",
  function (assert) {
...
QUnit.test(
  "Serializer.getObjPropertyValue/setObjPropertyValue for arrays",
  function (assert) {
    ...
    assert.deepEqual(
      question.tagbox,
      [1, 2],
      "do not change on chaning array outside"
    );
...
QUnit.test(
  "Edit survey title property with isLocalizable=false",
  function (assert) {
    ...
    const survey = new SurveyModel({
      elements: [{ type: "comment", name: "title" }],
    });
    survey.editingObj = editableSurvey;
    assert.equal(
      editableSurvey.title,
      "default-value",
      "editableSurvey.title loaded correctly"
    );
    assert.deepEqual(
      editableSurvey.locTitle.getJson(),
      "default-value",
      "editableSurvey.locTitle.getJson()"
    );
    assert.equal(
      editableSurvey.locTitle.text,
      "default-value",
      "editableSurvey.locTitle.text"
    );
    assert.equal(
      Serializer.getObjPropertyValue(editableSurvey, "title"),
      "default-value",
...
QUnit.test(
  "Edit question title property, setup initial value",
  function (assert) {
    var question = new QuestionTextModel("q1");
    question.title = "My title";
    var survey = new SurveyModel({
    ...
    survey.editingObj = question;
    assert.equal(
      survey.getValue("title"),
      "My title",
      "The title set correctly"
    );
...
QUnit.test("Edit columns in matrix", function (assert) {
  ...
  assert.equal(rows[0].cells[0].value, "col1", "Name set correctly");
  assert.notOk(rows[0].cells[1].value, "Title is empty");
  assert.equal(rows[1].cells[1].value, "Column 2", "Set title correctly");
...
QUnit.test("Edit columns in matrix, check for empty titles", function (assert) {
  ...
  const createSurveyAndSetColumn = (
...
QUnit.test("allowRowsDragAndDrop and editingObj", function (assert) {
  ...
  assert.strictEqual(
    ...
    table,
    "rendered table is not recreated, #1"
  );
  table = matrix.renderedTable;
  assert.equal(
    table.rows[1].cells[1].question.value,
    "col2",
    "renderedTable:Row0Cell0, #1"
  );
  assert.equal(
    table.rows[3].cells[1].question.value,
    "col3",
    "renderedTable:Row1Cell0, #1"
  );
  assert.equal(
    table.rows[5].cells[1].question.value,
    "col1",
    "renderedTable:Row1Cell0, #1"
  );
  ...
  assert.strictEqual(
    matrix.renderedTable,
    table,
    "rendered table is not recreated, #2"
  );
  table = matrix.renderedTable;
  assert.equal(
    table.rows[1].cells[1].question.value,
    "col3",
    "renderedTable:Row0Cell0, #2"
  );
  assert.equal(
    table.rows[3].cells[1].question.value,
    "col2",
    "renderedTable:Row1Cell0, #2"
  );
  assert.equal(
    table.rows[5].cells[1].question.value,
    "col1",
...
QUnit.test(
  "Do not re-create rows and rendered table on adding new choice item",
  function (assert) {
    var question = new QuestionDropdownModel("q1");
    question.choices = [{ value: "item1" }, { value: "item2", text: "Item 2" }];
    var survey = new SurveyModel({
...
QUnit.test(
  "Edit choices in matrix + detailPanel + addChoice",
  function (assert) {
    var question = new QuestionDropdownModel("q1");
    var survey = new SurveyModel({
      elements: [
...
QUnit.test("Composite: create from code", function (assert) {
  ...
  assert.equal(
    question.choicesByUrl.url,
    "abc",
    "Url set correctly into question #2"
  );
...
QUnit.test("Edit question string[] property type", function (assert) {
  ...
  assert.equal(
    dataListQuestion.value,
    "abc\ndef",
    "Update edited question correctly"
  );
});
QUnit.test(
...
QUnit.test(
  "Edit choices in matrix dynamic column, unique in detail panel",
  function (assert) {
      ...
      elements: [
        {
          ...
          rowCount: 0,
          keyName: "value",
          columns: [{ cellType: "text", name: "text" }],
...
QUnit.test(
  "Edit choices in matrix dynamic column, check errors",
  function (assert) {
    const question = new QuestionDropdownModel("q1");
    question.choices = ["Item1", "Item2", "Item3"];
    const survey = new SurveyModel({
...
QUnit.test(
  "Edit choices in matrix dynamic column, set correct value",
  function (assert) {
    const question = new QuestionDropdownModel("q1");
    question.choices = ["Item1", "Item2", "Item3"];
    const survey = new SurveyModel({
...
QUnit.test("Call onMatrixCellCreated correctly, Bug#8873", function (assert) {
  ...
  survey.onMatrixCellCreated.add((sender, options) => {
    ...
    if (options.columnName === "text") {
      options.cellQuestion.placeholder =
        options.row.getQuestionByName("value").value;
    }
  });
  ...
  assert.equal(
    matrix.visibleRows[2].cells[1].question.placeholder,
    "Item 4",
    "placeholder is set"
  );
});
...
QUnit.test(
  "Check column min width property is set correctly to editor",
  function (assert) {
    const question = new QuestionMatrixDynamicModel("q1");
    question.addColumn("col1");
...
QUnit.test(
  "Allow to set empty string into localization string & string property with default value",
  function (assert) {
    const question = new QuestionDropdownModel("q1");
    const survey = new SurveyModel({
      elements: [
    ...
    assert.strictEqual(
      placeholderQuestion.value,
      "",
      "placeholder question value is empty"
    );
    assert.strictEqual(
      minWidthQuestion.value,
      "",
      "minWidth question value is empty"
    );
    assert.strictEqual(
      question.placeholder,
      "",
      "dropdown.placeholder value is empty"
    );
    assert.strictEqual(
      question.minWidth,
      "",
      "dropdown.minWidth value is empty"
    );
...
QUnit.test("multipletextitem title in detail panel", function (assert) {
  ...
  const matrix = <QuestionMatrixDynamicModel>(
    editSurvey.getQuestionByName("items")
  );
...
QUnit.test("reset property value", function (assert) {
  ...
  const comment = <QuestionMatrixDynamicModel>(
    editSurvey.getQuestionByName("labelTrue")
  );
  editSurvey.editingObj = question;
...
QUnit.test("paneldynamic. templateVisibleIf", function (assert) {
  ...
  const comment = <QuestionMatrixDynamicModel>(
    editSurvey.getQuestionByName("templateVisibleIf")
  );
  editSurvey.editingObj = question;
...
QUnit.test("survey, complete text & locale", function (assert) {
  ...
  const commentHtml = <QuestionCommentModel>(
    editSurvey.getQuestionByName("completedHtml")
  );
  const commentText = <QuestionCommentModel>(
    editSurvey.getQuestionByName("description")
  );
  editSurvey.editingObj = survey;
...
QUnit.test("Column visible property", function (assert) {
  ...
  assert.equal(
    Serializer.getObjPropertyValue(column, "visible"),
    false,
    "Serializer.getObjPropertyValue"
  );
  ...
  assert.equal(
    Serializer.getObjPropertyValue(column, "visible"),
    true,
    "Serializer.getObjPropertyValue, #2"
  );
  column.visible = false;
  ...
  assert.equal(
    Serializer.getObjPropertyValue(column, "visible"),
    false,
    "Serializer.getObjPropertyValue, #3"
  );
});
