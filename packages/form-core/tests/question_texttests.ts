...
QUnit.test(
  "Test renderedPlaceholder on locale change, bug#7911",
  function (assert) {
...
QUnit.test("min date error text, bug #4596", function (assert) {
  const survey = new SurveyModel({
    elements: [
      { type: "text", name: "q1", inputType: "date", min: "2000-10-10" },
    ],
  });
  ...
  assert.equal(
    errorText.indexOf(":"),
    -1,
    "There is no time in the error text"
  );
});
QUnit.test("min/max onSettingValue property function", function (assert) {
  ...
  assert.equal(
    q.min,
    "2020-10-10",
    "Set the min date value, max value is empty"
  );
...
QUnit.test(
  "month value should have format 9999-99, Bug#8668",
  function (assert) {
    const q = new QuestionTextModel("q1");
    q.inputType = "month";
    q.value = "2024-01";
...
QUnit.test("Test event handlers", function (assert) {
  ...
  assert.equal(
    q.value,
    "test1",
    "value should be updated on change only if element is not focused #5270"
  );
...
QUnit.test(
  "Test event handlers with on typing text update mode",
  function (assert) {
    const testInput = document.createElement("input");
    const fakeInput = document.createElement("input");
    document.body.appendChild(testInput);
    ...
    assert.equal(
      q.value,
      "test2",
      "value always should be update on change if onTyping mode"
    );
    testInput.value = "test3";
    q.onBlur({
      target: testInput,
    ...
    assert.equal(
      q.value,
      "test4",
      "value should not be updated on key up if is waiting for enter and key is not enter"
    );
    q.onKeyUp({
      target: testInput,
      keyCode: 13,
    ...
    assert.equal(
      q.value,
      "test5",
      "value should be updated on key up if is waiting for enter and key is enter"
    );
...
QUnit.test(
  "Test event handlers do not change question'value if newValue is same",
  function (assert) {
    let log = "";
    const testInput = document.createElement("input");
    ...
    assert.equal(
      log,
      "->changedValue",
      "Value should be changed only one time"
    );
...
QUnit.test("min/max numeric, non required, bug#5758", function (assert) {
  const survey = new SurveyModel({
    elements: [
      { type: "text", name: "q1", inputType: "number", min: 1, max: 10 },
    ],
  });
...
QUnit.test(
  "CharacterCounter + settings.showMaxLengthIndicator",
  function (assert) {
...
QUnit.test("getControlClass with characterCounter", function (assert) {
  ...
  assert.equal(
    q.getControlClass(),
    inputClasses + " " + constrolWithCharacterCounter,
    "#2"
  );
...
QUnit.test("Text Question KeyHandler exists", function (assert) {
  const q = new QuestionTextModel("q1");
  assert.ok(
    q["onTextKeyDownHandler"],
    "we need this handler for using in Survey Creator"
  );
});
...
QUnit.test("inputType='date' invalid value, #8617", function (assert) {
  const survey = new SurveyModel({
    elements: [{ type: "text", name: "q1", inputType: "date" }],
  });
  const q1 = <QuestionTextModel>survey.getQuestionByName("q1");
  q1.value = "2000-01-01";
...
QUnit.test("Support mask in displayValue, #9268", function (assert) {
  ...
  assert.equal(
    q1.getDisplayValue(false, 1234),
    "1 234",
    "getDisplayValue with value #1"
  );
  q1.maskSettings.saveMaskedValue = true;
  assert.equal(
    q1.getDisplayValue(false, 1234),
    "1 234",
    "getDisplayValue with value #2"
  );
