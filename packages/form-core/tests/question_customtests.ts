...
QUnit.test("Single: Register and load from json", function (assert) {
  ...
  assert.equal(
    Serializer.getObjPropertyValue(q, "name"),
    "q1",
    "getObjPropertyValue is correct, #name"
  );
  assert.equal(
    Serializer.getObjPropertyValue(q, "title"),
    "my title",
    "getObjPropertyValue is correct, #title"
  );
  assert.equal(
...
QUnit.test("Composite: content questions numbering", function (assert) {
  var json = {
    ...
    onCreated: function (question) {
      question.contentPanel.getQuestionByName("lastName").startWithNewLine =
        false;
...
QUnit.test("Composite, update panel css", function (assert) {
  ...
  assert.equal(
    q1.cssRoot,
    "customercss small",
    "apply style from customer info"
  );
...
QUnit.test("Single: defaultValue", function (assert) {
  ComponentCollection.Instance.clear(); // Clear any existing components first
  var json = {
    name: "newquestion",
    questionJSON: {
      type: "dropdown",
...
QUnit.test("Single: defaultValue + valueName", function (assert) {
  ...
  var survey = new SurveyModel({
    elements: [
      { type: "newquestion", name: "q1", isRequired: true, valueName: "QQQ1" },
    ],
  });
  var q = <QuestionCustomModel>survey.getAllQuestions()[0];
  assert.equal(q.value, 2, "defaultValue is set");
...
QUnit.test(
  "Composite: defaultValue and survey in design mode",
  function (assert) {
      ...
      elements: [
        {
          ...
          name: "q1",
          isRequired: true,
          defaultValue: { firstName: "Jon", lastName: "Snow" },
        },
      ],
    });
    var q = <QuestionCompositeModel>survey.getAllQuestions()[0];
...
QUnit.test(
  "Composite: defaultValue and question.valueChangedCallback",
  function (assert) {
    var json = {
      name: "customerinfo",
      elementsJSON: [
      ...
      onCreated(question: QuestionCustomModel) {
        ...
        const firstQuestion =
          question.contentPanel.getQuestionByName("firstName");
        const lastQuestion =
          question.contentPanel.getQuestionByName("lastName");
...
QUnit.test("Single: defaultValueExpession, bug#4836", function (assert) {
  ...
  var survey = new SurveyModel({
    elements: [
      { type: "newquestion", name: "q1", defaultValueExpression: "2" },
      { type: "text", name: "q2" },
    ],
  });
  var q = <QuestionCustomModel>survey.getAllQuestions()[0];
  ...
  assert.equal(
    q.contentQuestion.value,
    3,
    "defaultValue is set for contentQuestion, #2"
  );
  ...
  assert.equal(
    q.contentQuestion.value,
    3,
    "defaultValue is set for contentQuestion, #3"
  );
  ...
  assert.equal(
    q.contentQuestion.value,
    2,
    "defaultValue is set for contentQuestion, #4"
  );
  assert.deepEqual(survey.data, { q1: 2, q2: 5 }, "set data into survey, #2");
  ComponentCollection.Instance.clear();
});
QUnit.test(
  "Single: defaultValueExpession & operations, bug#7280",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "newquestion",
      questionJSON: {
...
QUnit.test(
  "Single: onValueChanged function, value is array",
  function (assert) {
    var json = {
      name: "testquestion",
      questionJSON: {
...
QUnit.test("Composite: onValueChanging function", function (assert) {
  ...
  assert.equal(
    ...
    2,
    "onValueChanging works"
  );
  assert.deepEqual(
    q.value,
    { q2: 2 },
    "onValueChanging works, composite value"
  );
  ComponentCollection.Instance.clear();
});
QUnit.test("Composite: onMatrixCellValueChanging function", function (assert) {
  var json = {
    ...
    onCreated: (question: Question): void => {
      ...
      matrix.cellValueChangingCallback = (
        row: any,
        columnName: string,
        value: any
      ): any => {
  ...
  assert.deepEqual(
    survey.data,
    { q1: { q2: [{ col1: 2 }] } },
    "survey.data has correct values"
  );
  ComponentCollection.Instance.clear();
});
QUnit.test(
  "Single: onValueChanging function, value is array",
  function (assert) {
    var json = {
      name: "testquestion",
      questionJSON: {
...
QUnit.test("getUsedLocale, Bug#7510", function (assert) {
  ...
  assert.deepEqual(
    ...
    ["en", "de", "fr", "it"],
    "Pick-up locales from components"
  );
  ComponentCollection.Instance.clear();
});
QUnit.test("getDisplayValue from component JSON function", function (assert) {
  var json = {
    ...
    getDisplayValue: (composite: PanelModel) =>
      composite.getValue().firstName + " " + composite.getValue().lastName,
  };
  ...
  assert.equal(
    q.displayValue,
    "First Last",
    "Obtain passed getDisplayValue function result"
  );
  ComponentCollection.Instance.clear();
});
QUnit.test(
  "Complex: panel dynamic should duplicate rows in designMode",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "multiple_panel",
      elementsJSON: [
    ...
    var panel = <QuestionPanelDynamicModel>(
      q.contentPanel.getQuestionByName("panel")
    );
    panel.panelCount = 1;
...
QUnit.test("onvalueChanging/Changed events", function (assert) {
  ...
  question.value = "b";
  assert.equal(
    question.contentQuestion.value,
    "b",
    "contentQuestion value is changed"
  );
...
QUnit.test(
  "Single: survey.questionsOnPageMode = `singlePage`",
  function (assert) {
    const json = {
      name: "newquestion",
      questionJSON: { type: "dropdown", choices: [1, 2, 3, 4, 5] },
    ...
    assert.equal(
      q.contentQuestion.value,
      3,
      "content question value is correct"
    );
    ComponentCollection.Instance.clear();
  }
);
QUnit.test("Composite: in matrices cells", function (assert) {
    ...
    elements: [
      {
        ...
        name: "matrix",
        rowCount: 1,
        columns: [{ cellType: "customerinfo", name: "col1" }],
      },
    ],
  });
  ...
  assert.deepEqual(
    q.value,
    { firstName: "Jon", lastName: "Snow" },
    "Set value to composite question correctly"
  );
  assert.deepEqual(
    row.value,
    { col1: { firstName: "Jon", lastName: "Snow" } },
    "Row value is correct"
  );
  assert.deepEqual(
    matrix.value,
    [{ col1: { firstName: "Jon", lastName: "Snow" } }],
    "Matrix value is correct"
  );
  assert.deepEqual(
    survey.data,
    { matrix: [{ col1: { firstName: "Jon", lastName: "Snow" } }] },
  ...
  assert.equal(
    q.contentPanel.getQuestionByName("firstName").value,
    "Jaime",
    "row 0, firstName"
  );
  assert.equal(
...
QUnit.test("Single: isContentElement property", function (assert) {
  ...
  var q = <QuestionCustomModel>survey.getAllQuestions()[0];
  assert.equal(q.isContentElement, false, "Design is available for root");
  assert.equal(
...
QUnit.test("Single: text & getProgressInfo", function (assert) {
  ...
  const survey = new SurveyModel({
    elements: [
      { type: "test1", name: "q1", isRequired: true },
      { type: "test2", name: "q2" },
    ],
  });
...
QUnit.test("Composite: Support carry-forward", function (assert) {
  ...
  assert.equal(
    q2.choicesFromQuestionMode,
    "selected",
    "choicesFromQuestionMode is loaded"
  );
...
QUnit.test("Single: Change css rules for content question", function (assert) {
  ...
  const q = <QuestionCustomModel>survey.getAllQuestions()[0];
  assert.equal(
    q.contentQuestion.cssClasses.root,
    "css_question",
    "Set the css correctly"
  );
  ComponentCollection.Instance.clear();
});
QUnit.test(
  "Composite: Change css rules for content questions",
  function (assert) {
    const json = {
      name: "fullname",
      elementsJSON: [
    ...
    var q = <QuestionCompositeModel>survey.getAllQuestions()[0];
    assert.equal(
      q.contentPanel.questions[0].cssClasses.root,
      "css_question1",
      "Set the css correctly, #1"
    );
    assert.equal(
      q.contentPanel.questions[1].cssClasses.root,
      "css_question2",
      "Set the css correctly, #2"
    );
    ComponentCollection.Instance.clear();
  }
);
...
QUnit.test(
  "Composite: with setValueIf & setValueExpression, bug#7888",
  function (assert) {
    const json = {
      name: "comp1",
      elementsJSON: [
    ...
    ComponentCollection.Instance.add(json);
    const survey = new SurveyModel({
      elements: [{ type: "comp1", name: "question1" }],
    });
    const q = <QuestionCompositeModel>survey.getAllQuestions()[0];
    const q1 = q.contentPanel.getQuestionByName("q1");
    const q2 = q.contentPanel.getQuestionByName("q2");
...
QUnit.test(
  "Composite: check valueToData and valueFromData callbacks",
  function (assert) {
    const json = {
      name: "test",
      questionJSON: {
...
QUnit.test(
  "Composite & valueToQuestion/valueFromQuestion, #6475",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "elementsettings",
      showInToolbox: false,
    ...
    assert.equal(
      q1.value,
      "val1,val2",
      "composite question value is correct, #1"
    );
    assert.deepEqual(
      survey.data,
      { q1: "val1,val2" },
      "survey data is correct, #1"
    );
    q1.value = "val3,val4";
    ...
    assert.equal(
      q1.value,
      "val3,val4",
      "composite question value is correct, #2"
    );
    assert.deepEqual(
      survey.data,
      { q1: "val3,val4" },
      "survey data is correct, #2"
    );

    const q2 = <QuestionCompositeModel>survey.getQuestionByName("q2");
    const q2Settings = <QuestionCompositeModel>(
      q2.contentPanel.getQuestionByName("settings")
    );
    ...
    assert.equal(
      q2SettingsItem1.value,
      "val5",
      "item1 question value is correct, #3"
    );
    assert.equal(
      q2SettingsItem2.value,
      "val6",
      "item2 question value is correct, #3"
    );
    assert.equal(
      q2Settings.value,
      "val5,val6",
      "composite question value is correct, #3"
    );
    assert.deepEqual(
      q2.value,
      { settings: "val5,val6" },
      "composite root question value is correct, #3"
    );
    assert.deepEqual(
      survey.data,
      { q1: "val3,val4", q2: { settings: "val5,val6" } },
      "survey data is correct, #3"
    );
    q2Settings.value = "val7,val8";
    assert.equal(
      q2SettingsItem1.value,
      "val7",
      "item1 question value is correct, #4"
    );
    assert.equal(
      q2SettingsItem2.value,
      "val8",
      "item2 question value is correct, #4"
    );
    assert.equal(
      q2Settings.value,
      "val7,val8",
      "composite question value is correct, #4"
    );
    assert.deepEqual(
      q2.value,
      { settings: "val7,val8" },
      "composite root question value is correct, #4"
    );
    assert.deepEqual(
      survey.data,
      { q1: "val3,val4", q2: { settings: "val7,val8" } },
      "survey data is correct, #4"
    );
    q2.value = { settings: "val9,val10" };
    assert.equal(
      q2SettingsItem1.value,
      "val9",
      "item1 question value is correct, #5"
    );
    assert.equal(
      q2SettingsItem2.value,
      "val10",
      "item2 question value is correct, #5"
    );
    assert.equal(
      q2Settings.value,
      "val9,val10",
      "composite question value is correct, #5"
    );
    assert.deepEqual(
      q2.value,
      { settings: "val9,val10" },
      "composite root question value is correct, #5"
    );
    assert.deepEqual(
      survey.data,
      { q1: "val3,val4", q2: { settings: "val9,val10" } },
      "survey data is correct, #5"
    );
...
QUnit.test("needResponsiveWidth", function (assert) {
  ...
  assert.equal(
    survey.getQuestionByName("q1").needResponsiveWidth(),
    true,
    "single - matrix"
  );
  assert.equal(
    survey.getQuestionByName("q2").needResponsiveWidth(),
    false,
    "single - text"
  );
  assert.equal(
    survey.getQuestionByName("q3").needResponsiveWidth(),
    false,
    "complex - text"
  );
  assert.equal(
    survey.getQuestionByName("q4").needResponsiveWidth(),
    true,
    "single - matrix"
  );
...
QUnit.test("Single: onHidingContent", function (assert) {
  ...
  ComponentCollection.Instance.add({
    ...
    questionJSON: { type: "text" },
    onCreated: (question: Question): void => {
      question.contentQuestion.onHidingContent = (): void => {
        counter++;
      };
    },
  });
...
QUnit.test("Complex: onHidingContent", function (assert) {
  ...
  ComponentCollection.Instance.add({
    ...
    onCreated: (question: Question): void => {
      ...
      questions[0].onHidingContent = (): void => {
        counter++;
      };
      questions[1].onHidingContent = (): void => {
        counter++;
      };
    },
  });
...
QUnit.test("Single: Apply error css", function (assert) {
  ...
  assert.equal(
    qText.getControlClass().indexOf(errorCss) < 0,
    true,
    "errors is not here"
  );
  q.validate(true);
  assert.equal(
    qText.getControlClass().indexOf(errorCss) > -1,
    true,
    "errors is here"
  );
  ComponentCollection.Instance.clear();
...
QUnit.test("ComponentCollection.Instance.remove", function (assert) {
  ...
  assert.equal(
    ComponentCollection.Instance.getCustomQuestionByName("newquestion").name,
    "newquestion",
    "it exists"
  );
  assert.equal(
    ComponentCollection.Instance.remove("aaa"),
    false,
    "aaa is not exists"
  );
  assert.equal(
    ComponentCollection.Instance.remove("newquestion"),
    true,
    "newquestion is removed"
  );
  assert.notOk(
    ComponentCollection.Instance.getCustomQuestionByName("newquestion"),
    "newquestion is not here"
  );
});
QUnit.test("internal boolean flag", function (assert) {
  ...
  assert.equal(
    ComponentCollection.Instance.getCustomQuestionByName("newquestion1").name,
    "newquestion1",
    "newquestion1 is here"
  );
  assert.equal(
    ComponentCollection.Instance.getCustomQuestionByName("newquestion2").name,
    "newquestion2",
    "newquestion2 is here"
  );
  ComponentCollection.Instance.clear();
  assert.equal(
    ComponentCollection.Instance.getCustomQuestionByName("newquestion1").name,
  ...
  assert.notOk(
    ComponentCollection.Instance.getCustomQuestionByName("newquestion2"),
    "newquestion2 is not here, #1"
  );
  ComponentCollection.Instance.clear(true);
  assert.notOk(
    ComponentCollection.Instance.getCustomQuestionByName("newquestion1"),
    "newquestion1 is not here, #2"
  );
  assert.notOk(
    ComponentCollection.Instance.getCustomQuestionByName("newquestion2"),
    "newquestion2 is not here, #2"
  );
});
...
QUnit.test(
  "Allow to add question via addNewQuestion for component, but not for abstract classes",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "newquestion",
      questionJSON: { type: "text", title: "Title from Component" },
...
QUnit.test(
  "text placeholder is not updated on changing locale",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "customtext",
      questionJSON: {
    ...
    const survey = new SurveyModel({
      elements: [{ type: "customtext", name: "q1" }],
    });
    const q1 = <QuestionCustomModel>survey.getQuestionByName("q1");
    const contentQuestion = <QuestionTextModel>q1.contentQuestion;
    assert.equal(
      contentQuestion.renderedPlaceholder,
      "en-TextPH",
      "en placeholder"
    );
    survey.locale = "de";
...
QUnit.test("showPreview & default value, #7508", function (assert) {
  ...
  assert.equal(
    contentQuestionPreview.value,
    "edf",
    "contentQuestionPreview.value #3"
  );
  survey.cancelPreview();
...
QUnit.test("showPreview & default value, #7640", function (assert) {
  ...
  assert.equal(
    contentQuestion.locTitle.renderedHtml,
    "abc=123",
...
QUnit.test(
  "single component: defaultQuestionTitle & editor placeholder",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "customtext",
...
QUnit.test(
  "single component: inheritBaseProps: array<string>",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "customdropdown",
...
QUnit.test(
  "single component: inheritBaseProps: array<string> #2 + check property change notification #",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "customtext",
    ...
    assert.equal(
      propertyName,
      "placeholder",
      "send notification, propertyname"
    );
...
QUnit.test("single component: inheritBaseProps: true", function (assert) {
  ...
  const q1 = <QuestionCustomModel>survey.getQuestionByName("q1");
  assert.equal(
    Serializer.getObjPropertyValue(q1, "name"),
    "q1",
    "getObjPropertyValue is correct, #name"
  );
  assert.equal(
    Serializer.getObjPropertyValue(q1, "title"),
    "my title",
    "getObjPropertyValue is correct, #title"
  );
  assert.equal(
    Serializer.getObjPropertyValue(q1, "showOtherItem"),
    true,
    "getObjPropertyValue is correct, #showOtherItem"
  );
  const content = <QuestionDropdownModel>q1.contentQuestion;
...
QUnit.test(
  "single component, file: inheritBaseProps: true, Bug #8757",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "customfile",
      inheritBaseProps: true,
    ...
    const survey = new SurveyModel({
      elements: [{ type: "customfile", name: "q1", showCommentArea: true }],
    });
    const q1 = <QuestionCustomModel>survey.getQuestionByName("q1");
    assert.equal(
      Serializer.getObjPropertyValue(q1, "name"),
    ...
    assert.equal(
      Serializer.getObjPropertyValue(q1, "showCommentArea"),
      true,
      "getObjPropertyValue is correct, #showCommentArea"
    );
...
QUnit.test(
  "Bug with visibleIf with composite.question and panel dynamic. Bug#7771",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "test",
      elementsJSON: [
    ...
    const q1 = <QuestionPanelDynamicModel>(
      compQuestion.contentPanel.getQuestionByName("q1")
    );
    const q2 = <QuestionPanelDynamicModel>(
      compQuestion.contentPanel.getQuestionByName("q2")
    );
...
QUnit.test(
  "file question in composite component doesn't show preview in preview mode. Bug#7826",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "test",
      elementsJSON: [
    ...
    const compQuestion = <QuestionCompositeModel>survey.getQuestionByName("q1");
    const file_q1 = <QuestionFileModel>(
      compQuestion.contentPanel.getQuestionByName("file_q")
    );
    ...
    const compQuestion_preview = <QuestionCompositeModel>(
      survey.getQuestionByName("q1")
    );
    const file_q1_preview = <QuestionFileModel>(
      compQuestion_preview.contentPanel.getQuestionByName("file_q")
    );
    const file_q2_preview = <QuestionFileModel>survey.getQuestionByName("q2");
    assert.equal(
      file_q1_preview.showPreviewContainer,
      true,
      "file_q1_preview #1"
    );
    assert.equal(
      file_q2_preview.showPreviewContainer,
      true,
      "file_q2_preview #1"
    );
...
QUnit.test("Composite + Ranking", function (assert) {
  ...
  assert.equal(
    ...
    2,
    "ranking value is correct (unrank list length) ['d', 'e']"
  );
  assert.equal(
    q_ranking.rankingChoices.length,
    3,
    "ranking value is correct (rank list length) ['a', 'b', 'c'"
  );
  ...
  ComponentCollection.Instance.add({
    ...
    elementsJSON: [
      { type: "text", name: "q1" },
      {
        ...
        name: "q2",
        choices: [1, 2, 3],
        visibleIf: "{composite.q1} notempty",
      },
      {
        type: "text",
        name: "q3",
        choices: [1, 2, 3],
        visibleIf: "{composite.q2} notempty",
      },
    ],
    onValueChanged(question, name, newValue) {
      if (name === "q1") {
...
QUnit.test(
  "Composite: onValueChanging and survey.onValueChanging",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "test",
      elementsJSON: [
...
QUnit.test("Single: use incorrect json", function (assert) {
  ...
  assert.deepEqual(
    reportTexts,
    [
      "Could not create component: 'test1'. questionJSON should be a question.",
...
QUnit.test(
  "single component: inheritBaseProps - do not duplicate description property",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "customdropdown",
    ...
    const survey = new SurveyModel({
      elements: [{ type: "customdropdown", name: "q1" }],
    });
    const q1 = <QuestionCustomModel>survey.getQuestionByName("q1");
    assert.equal(
      q1.description,
      "Custom Question",
      "Get description from internal description"
    );
    survey.locale = "de";
    assert.equal(
      q1.description,
      "Aangepaste vraag",
      "Get description from internal description for 'de'"
    );
...
QUnit.test(
  "composite component: do not reset dynamic panels/dynamic rows, Bug#8612",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "customquestion",
      elementsJSON: [
    ...
    const survey = new SurveyModel({
      elements: [{ type: "customquestion", name: "q1" }],
    });
    const q1 = <QuestionCompositeModel>survey.getQuestionByName("q1");
    const panel = <QuestionPanelDynamicModel>(
      q1.contentPanel.getQuestionByName("panel")
    );
    const matrix = <QuestionMatrixDynamicModel>(
      q1.contentPanel.getQuestionByName("matrix")
    );
    ...
    elements: [
      {
        type: "nps",
        name: "q1",
        minRateDescription: "val1",
        maxRateDescription: "val2",
      },
    ],
  });
  const q1 = <QuestionCustomModel>survey.getQuestionByName("q1");
  assert.equal(
    q1.contentQuestion.locMinRateDescription.text,
    "val1",
    "minRateDescription"
  );
  assert.equal(
    q1.contentQuestion.maxRateDescription,
    "val2",
    "maxRateDescription"
  );
  assert.equal(
    q1.contentQuestion.hasMinRateDescription,
    true,
    "hasMinRateDescription"
  );
  assert.equal(
    q1.contentQuestion.hasMaxRateDescription,
    true,
...
QUnit.test("Dynamic serializable properties, bug#8852", function (assert) {
  ...
  const survey = new SurveyModel({
    elements: [{ type: "dropdownWithComment", name: "q1", choices: [1] }],
  });
...
QUnit.test(
  "Composite: with dropdown & showOtherItem, Bug#9378",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "test",
      elementsJSON: [
    ...
    assert.deepEqual(
      q.value,
      { q1: "test1", q2: "other", "q2-Comment": "abc" },
      "q.value #1"
    );
    survey.data = {};
    assert.ok(q.isEmpty(), "q.value #2");
    survey.data = {
      question1: { q1: "test2", q2: "other", "q2-Comment": "def" },
    };
    assert.deepEqual(
      q.value,
      { q1: "test2", q2: "other", "q2-Comment": "def" },
      "q.value #3"
    );
...
QUnit.test(
  "Composite: checkErrorsMode: `onComplete` with several elements, Bug#9361",
  function (assert) {
    ComponentCollection.Instance.add({
      name: "test",
      elementsJSON: [
    ...
    const survey = new SurveyModel({
      ...
      pages: [
        { name: "page1", elements: [{ type: "test", name: "question1" }] },
        { name: "page2", elements: [{ type: "test", name: "question2" }] },
      ],
    });
    const question1 = <QuestionCompositeModel>(
