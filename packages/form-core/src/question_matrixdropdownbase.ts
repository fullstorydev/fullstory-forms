...
} from "./question_matrixdropdowncolumn";
import {
  QuestionMatrixDropdownRenderedCell,
  QuestionMatrixDropdownRenderedRow,
  QuestionMatrixDropdownRenderedTable,
} from "./question_matrixdropdownrendered";
...
export class MatrixDropdownCell {
  ...
  private updateCellQuestionTitleDueToAccessebility(
    ...
    this.questionValue.locTitle.onGetTextCallback = (str: string): string => {
      ...
      return this.column.colOwner.getCellAriaLabel(
        rowTitle,
...
class MatrixDropdownRowTextProcessor extends QuestionTextProcessor {
  ...
  protected onCustomProcessText(textValue: TextPreProcessorValue): boolean {
    ...
    if (
      [
        MatrixDropdownRowModelBase.RowValueVariableName,
        MatrixDropdownRowModelBase.RowNameVariableName,
      ].indexOf(textValue.name) > -1
    ) {
      textValue.isExists = true;
  ...
  implements ISurveyData, ISurveyImpl, ILocalizableOwner
{
  ...
  constructor(data: IMatrixDropdownData, value: any) {
    ...
    this.textPreProcessor = new MatrixDropdownRowTextProcessor(
      ...
      !!data ? data.getParentTextProcessor() : null
    );
  ...
  private ensureDetailPanel() {
    ...
    if (!Helpers.isValueEmpty(value)) {
      for (var i = 0; i < questions.length; i++) {
        ...
        const val = !!this.editingObj
          ? Serializer.getObjPropertyValue(this.editingObj, key)
          : value[key];
  ...
  public runCondition(
    ...
    if (!this.data) return;
    values[MatrixDropdownRowModelBase.OwnerVariableName] =
    ...
    const rowValues =
      rowIndex > 0 ? this.data.getRowValue(this.rowIndex - 1) : this.value;
    if (!!rowsVisibleIf) {
  ...
  private setValueCore(name: string, newColumnValue: any, isComment: boolean) {
    ...
    if (changedName) {
      this.runTriggers(
        MatrixDropdownTotalRowModel.RowVariableName + "." + changedName,
        newValue
      );
    }
  ...
  public get elementData(): any {
    const data = {
      ...
      "fs-table-row-index": this.rowIndex,
      "fs-table-row-name": this.rowName,
    };
...
export class QuestionMatrixDropdownModelBase
  extends QuestionMatrixBaseModel<
    ...
    MatrixDropdownColumn
  >
  implements IMatrixDropdownData, IMatrixColumnOwner
{
  ...
  public getChildErrorLocation(child: Question): string {
    const errLocation = !!child.parent
      ? this.detailErrorLocation
      : this.cellErrorLocation;
  ...
  public get useCaseSensitiveComparison(): boolean {
    return this.useCaseSensitiveComparisonValue !== undefined
      ? this.useCaseSensitiveComparisonValue
      : settings.comparator.caseSensitive;
  }
  ...
  public clearErrors(): void {
    super.clearErrors();
    this.runFuncForCellQuestions((q: Question) => {
      q.clearErrors();
    });
  }
  ...
  protected runConditionsForColumns(
    values: HashTable<any>,
    properties: HashTable<any>
  ): boolean {
  ...
  private checkColumnsVisibility(): void {
    ...
    for (var i = 0; i < this.columns.length; i++) {
      ...
      const isCellsVisibilty =
        !!column.visibleIf || column.isFilteredMultipleColumns;
      if (!isCellsVisibilty && !this.columnsVisibleIf && column.isColumnVisible)
        continue;
      hasChanged =
        this.isColumnVisibilityChanged(column, isCellsVisibilty) || hasChanged;
    }
    if (hasChanged) {
      this.resetRenderedTable();
    }
  ...
  private isColumnVisibilityChanged(
    column: MatrixDropdownColumn,
    checkCellsVisiblity: boolean
  ): boolean {
    const curVis = column.isColumnVisible;
    let hasVisCell = !checkCellsVisiblity;
    const rows = this.generatedVisibleRows;
    const checkRows = checkCellsVisiblity && rows;
    const isMultipleColumnsVisibility =
      checkRows && column.isFilteredMultipleColumns;
    const curVisibleChoices = isMultipleColumnsVisibility
      ? column.getVisibleChoicesInCell
      : [];
  ...
  private getVisibleFromGenerated(
    rows: Array<MatrixDropdownRowModelBase>
  ): Array<MatrixDropdownRowModelBase> {
  ...
  private updateValueOnRowsGeneration(rows: Array<MatrixDropdownRowModelBase>) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      newValue = this.getNewValueOnRowChanged(
        row,
        "",
        rValue,
        false,
  ...
  public addConditionObjectsByContext(
    ...
    for (var i = 0; i < rowsIndeces.length; i++) {
      ...
      const rowName =
        index > -1 ? this.getConditionObjectRowName(index) : "row";
      if (!rowName) continue;
      const rowText =
        index > -1 ? this.getConditionObjectRowText(index) : "row";
      const hasQuestionPrefix = index > -1 || context === true;
      const dot = hasQuestionPrefix && index === -1 ? "." : "";
      const prefixName =
        (hasQuestionPrefix ? this.getValueName() : "") + dot + rowName + ".";
      const prefixTitle =
        (hasQuestionPrefix ? this.processedTitle : "") + dot + rowText + ".";
  ...
  protected collectNestedQuestonsInRows(
    rows: Array<MatrixDropdownRowModelBase>,
    questions: Question[],
    visibleOnly: boolean
  ): void {
  ...
  supportAutoAdvance(): boolean {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      for (var colIndex = 0; colIndex < cells.length; colIndex++) {
        var question = cells[colIndex].question;
        if (question && (!question.supportAutoAdvance() || !question.value))
          return false;
      }
    }
    return true;
  ...
  private hasErrorInRows(fireCallback: boolean, rec: any): boolean {
    ...
    for (var i = 0; i < rows.length; i++) {
      if (rows[i].isVisible) {
        res =
          rows[i].hasErrors(fireCallback, rec, () => {
  ...
  private showDuplicatedErrorsInRows(
    duplicatedRows: Array<MatrixDropdownRowModelBase>,
    columnName: string
  ): void {
  ...
  onRowChanging(
    ...
    if (!!this.cellValueChangingCallback) {
      options.value = this.cellValueChangingCallback(
        row,
        columnName,
        options.value,
        options.oldValue
  ...
  setIsDetailPanelShowing(row: MatrixDropdownRowModelBase, val: boolean): void {
    ...
    if (this.survey) {
      this.survey.matrixDetailPanelVisibleChanged(
        this,
        row.rowIndex - 1,
        row,
        val
  ...
  public getDetailPanelButtonCss(row: MatrixDropdownRowModelBase): string {
    const builder = new CssClassBuilder().append(
      this.getPropertyValue("detailButtonCss" + row.id)
    );
    return builder
      .append(this.cssClasses.detailButton, builder.toString() === "")
      .toString();
  }
  public getDetailPanelIconCss(row: MatrixDropdownRowModelBase): string {
    const builder = new CssClassBuilder().append(
      this.getPropertyValue("detailIconCss" + row.id)
    );
    return builder
      .append(this.cssClasses.detailIcon, builder.toString() === "")
      .toString();
  }
  public getDetailPanelIconId(row: MatrixDropdownRowModelBase): string {
    return this.getIsDetailPanelShowing(row)
  ...
  private updateDetailPanelButtonCss(row: MatrixDropdownRowModelBase) {
    ...
    const iconBuilder = new CssClassBuilder()
      .append(classes.detailIcon)
      .append(classes.detailIconExpanded, isPanelShowing);
    this.setPropertyValue("detailIconCss" + row.id, iconBuilder.toString());

    const buttonBuilder = new CssClassBuilder()
      .append(classes.detailButton)
  ...
  protected clearInvisibleValuesInRows(): void {
    ...
    const sharedQuestions =
      this.survey?.questionsByValueName(this.getValueName()) || [];
  ...
  protected isRowsFiltered(): boolean {
    return (
      super.isRowsFiltered() || this.visibleRows !== this.generatedVisibleRows
    );
  }
  ...
  public getCellWrapperComponentName(cell: MatrixDropdownCell) {
    return this.SurveyModel.getElementWrapperComponentName(
      cell,
      cell.row instanceof MatrixDropdownTotalRowModel ? "row-footer" : "cell"
    );
  }
...
Serializer.addClass(
  ...
  [
    ...
    { name: "horizontalScroll:boolean", visible: false },
    {
      ...
      visibleIf: (obj): boolean => obj.isSelectCellType(),
    },
    {
      name: "placeholder",
