...
export class SurveyElementBase<P, S> extends React.Component<P, S> {
  ...
  public static renderQuestionDescription(question: Question | PanelModel): React.JSX.Element {
    ...
    return (
      <div
        style={question.hasDescription ? undefined : { display: "none" }}
        id={question.ariaDescriptionId}
        className={question.cssDescription}
      >
        {descriptionText}
      </div>
    );
  ...
  componentDidUpdate(prevProps: any, prevState: any) {
    ...
    this.disableStateElementsRerenderEvent(
      (this.prevStateElements ?? []).filter(el => !stateElements.find(stateElement => stateElement == el))
    );
  ...
  protected renderLocString(locStr: LocalizableString, style: any = null, key?: string): React.JSX.Element {
  ...
  private makeBaseElementReact(stateElement: Base) {
    ...
    stateElement.setPropertyValueCoreHandler = (hash: any, key: string, val: any) => {
      if (hash[key] !== val) {
        hash[key] = val;
        if (!this.canUsePropInState(key)) return;
        if (this.isRendering) return;
  ...
  public createElementData(obj: any, name: string = ""): { "data-fs-properties-schema": string; [v: string]: string } {
    // Find all keys
  ...
  //         data[name] = this.props["question"].title;
  ...
  //     }
  // }
  // getDataElementItem(elementName: string, value?: any, selected?: boolean) {
