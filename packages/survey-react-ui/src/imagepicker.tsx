...
export class SurveyQuestionImagePicker extends SurveyQuestionElementBase {
    ...
    protected renderElement(): React.JSX.Element {
        ...
        const dataElement = this.getDataElement("fs-image-picker");
        return (
            <fieldset {...dataElement} className={this.question.getSelectBaseRootCss()}>
    ...
    protected getColumns(cssClasses: any) {
        return this.question.columns.map((column: Array<ItemValue>, ci: number) => {
            var items = column.map((item: any, ii: number) => this.renderItem("item" + ii, item, cssClasses));
            return (
                <div
...
export class SurveyQuestionImagePickerItem extends ReactSurveyElement {
    ...
    private reactOnStrChanged() {
        this.item.locImageLink.onChanged = () => {
            this.setState({
                locImageLinkchanged: !!this.state && this.state.locImageLink ? this.state.locImageLink + 1 : 1
            });
        };
    }
    ...
    protected renderElement(): React.JSX.Element {
        ...
        if (!item.locImageLink.renderedHtml || item.contentNotLoaded) {
            ...
            control = (
                <div className={cssClasses.itemNoImage} style={style}>
        ...
        const dataElement = this.getDataElementItem("fs-image-picker-item", item.value, isChecked);
        const renderedItem = (
