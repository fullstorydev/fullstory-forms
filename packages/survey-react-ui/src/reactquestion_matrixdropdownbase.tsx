...
> {
    ...
    renderHeader(): React.JSX.Element | null {
        ...
        for (var i = 0; i < cells.length; i++) {
            ...
            const header = cell.hasTitle ? (
                <th className={cell.className} key={key} style={columnStyle}>
                    {" "}
                    {cellContent}{" "}
                </th>
            ) : (
    ...
    renderRow(
        ...
        const elementData = this.createElementData(data, "fs-table-row");
        ...
        return (
            <React.Fragment key={key}>
                {reason == "row-footer" ? (
                    ...
                    <MatrixRow model={row} parentMatrix={this.question} elementData={elementData}>
                        {matrixrow}
                    </MatrixRow>
                )}
            </React.Fragment>
    ...
    private renderCellContent(
        ...
        if (cell.hasTitle) {
            ...
            const require = !!cell.column ? (
                <SurveyQuestionMatrixHeaderRequired column={cell.column} question={this.question} />
            ) : null;
            cellContent = (
                <>
                    {str}
            ...
            cellContent = (
                <>
                    <SurveyQuestionMatrixDynamicDragDropIcon
                        item={{ data: { row: cell.row, question: this.question } }}
                    />
                </>
            );
        }
    ...
    protected renderElement(): React.JSX.Element | null {
        ...
        const elementData = this.getDataElement("fs-table");
        return (
            <table {...elementData} className={this.question.getTableCss()}>
                {header}
                {rows}
                {footers}
            </table>
        ...
        return (
            <div style={divStyle} className={this.question.cssClasses.tableWrapper} ref={root => this.setControl(root)}>
                <SurveyQuestionMatrixTable
                    question={this.question}
                    creator={this.creator}
                    wrapCell={(cell, element, reason) => this.wrapCell(cell, element, reason)}
                ></SurveyQuestionMatrixTable>
            </div>
        );
    }
...
class SurveyQuestionMatrixActionsCell extends ReactSurveyElement {
    ...
    protected renderElement(): React.JSX.Element {
        return <SurveyActionBar model={this.model} handleClick={false}></SurveyActionBar>;
    }
}
...
export class SurveyQuestionMatrixDropdownCell extends SurveyQuestionAndErrorsCell {
    ...
    protected renderCellContent() {
        ...
        const responsiveTitle = this.cell.showResponsiveTitle ? (
            <span className={this.cell.responsiveTitleCss}>
                {this.renderLocString(this.cell.responsiveLocTitle)}
                <SurveyQuestionMatrixHeaderRequired column={this.cell.column} question={this.cell.matrix} />
            </span>
        ) : null;
        return (
            <>
    ...
    protected renderQuestion(): React.JSX.Element {
        ...
        if (!this.cell.isChoice) return SurveyQuestion.renderQuestionBody(this.creator, this.question);
    ...
    private renderCellRadiogroupButton(): React.JSX.Element {
        var key = this.cell.question.id + "item" + this.cell.choiceIndex;
        return (
